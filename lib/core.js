...
const Core = module.exports = {
  ...
  bindApplicationListeners (app) {
    ...
    app.once('trailpack:all:initialized', () => {
      app.log.silly(app.config.get('motd.silly.initialized'))
      app.log.info(app.config.get('motd.info.initialized'))
    })
    app.once('trails:ready', () => {
      app.log.info(app.config.get('motd.info').ready(app))
      app.log.debug(app.config.get('motd.debug').ready(app))
      app.log.silly(app.config.get('motd.silly').ready(app))

      app.log.info(app.config.get('motd.hr'))
    })
    app.once('trails:stop', () => {
      app.log.silly(app.config.get('motd.silly').stop)
      app.log.info(app.config.get('motd.info').stop)
      app.unfreezeConfig()
    })

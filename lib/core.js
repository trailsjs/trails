...
const Core = module.exports = {
  ...
  buildConfig (initialConfig, nodeEnv) {
    ...
    return _.merge(configTemplate, initialConfig, (envConfig || { }))
  },
  ...
  validateConfig (config) {
    ...
    const nestedEnvs = Core.getNestedEnv(config)
  ...
  freezeConfig (config, modules) {
    ...
    propNames.forEach(name => {
      ...
      Core.freezeConfig(prop, modules)
    })
  ...
  bindApplicationListeners (app) {
    app.once('trailpack:all:configured', () => {
      if (app.config.main.freezeConfig !== false) {
        ...
        Core.freezeConfig(app.config, app.loadedModules)
      }
    })
    ...
    app.once('trails:stop', () => {
      ...
      Core.unfreezeConfig(app, app.loadedModules)
    })
